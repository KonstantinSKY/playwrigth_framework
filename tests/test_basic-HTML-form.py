...
def test_dropdown(page):
    ...
    option_locator = dropdown_locator.locator(f'option:has-text("{options_to_test[1]}")')
    ...
    for i in range(all_options.count()):
        print(all_options.nth(i).get_attribute('value'), all_options.nth(i).inner_text().strip())
